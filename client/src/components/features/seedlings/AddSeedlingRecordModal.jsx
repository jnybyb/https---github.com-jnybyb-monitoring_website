    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ color: '#2c5530', fontSize: '1.1rem', fontWeight: '600', margin: 0 }}>
            Select GPS Coordinates
          </h3>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.25rem', color: '#6c757d', padding: '0.25rem', borderRadius: '4px' }}>
            <IoClose />
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: '#2c5530', fontSize: '0.85rem', fontWeight: '500' }}>
              Latitude
            </label>
            <input type="number" step="any" placeholder="e.g., 7.2167" value={coordinates.lat} onChange={(e) => handleCoordinateChange('lat', e.target.value)} style={{ width: '100%', padding: '0.75rem', border: '1px solid #ddd', borderRadius: '4px', fontSize: '0.85rem' }} />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: '#2c5530', fontSize: '0.85rem', fontWeight: '500' }}>
              Longitude
            </label>
            <input type="number" step="any" placeholder="e.g., 126.3333" value={coordinates.lng} onChange={(e) => handleCoordinateChange('lng', e.target.value)} style={{ width: '100%', padding: '0.75rem', border: '1px solid #ddd', borderRadius: '4px', fontSize: '0.85rem' }} />
          </div>
        </div>

        <div style={{ backgroundColor: '#f8f9fa', padding: '1rem', borderRadius: '6px', marginBottom: '1rem' }}>
          <h4 style={{ color: '#2c5530', fontSize: '0.9rem', fontWeight: '600', marginBottom: '0.5rem' }}>Quick Actions</h4>
          <button onClick={handleGetCurrentLocation} style={{ padding: '0.5rem 1rem', backgroundColor: '#2d7c4a', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <IoLocation />
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '0.75rem 1.5rem', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '500' }}>
            Cancel
          </button>
          <button onClick={handleSaveCoordinates} style={{ padding: '0.75rem 1.5rem', backgroundColor: '#2d7c4a', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '500' }}>
            Save Coordinates
          </button>
        </div>
      </div>
...
const AddSeedlingRecordModal = ({ isOpen, onClose, onSubmit, record = null, isEdit = false }) => {
  ...
  useEffect(() => {
    if (isEdit && record) {
      ...
      setFormData({ beneficiaryId: '', beneficiaryName: '', received: '', planted: '', hectares: '', dateOfPlanting: '', gps: '' });
    }
    setErrors({});
  ...
  const validateForm = () => {
    ...
    if (!formData.dateOfPlanting) newErrors.dateOfPlanting = 'Date of planting is required';
    setErrors(newErrors);
  ...
  const handleCoordinateSelect = (coordinates) => {
    setFormData(prev => ({ ...prev, gps: coordinates }));
  };
  ...
  return (
    <>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <div>
                  ...
                  <input type="date" name="dateOfPlanting" value={formData.dateOfPlanting} onChange={handleInputChange} style={{ width: '100%', padding: '0.5rem', border: errors.dateOfPlanting ? '1px solid #dc3545' : '1px solid #ced4da', borderRadius: '4px', fontSize: '0.75rem', backgroundColor: 'white', color: '#495057', height: '36px' }} disabled={loading} />
                  {errors.dateOfPlanting && (<p style={{ color: '#dc3545', fontSize: '0.625rem', marginTop: '0.125rem' }}>{errors.dateOfPlanting}</p>)}
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end' }}>
                    <input type="text" name="gps" value={formData.gps} onChange={handleInputChange} style={{ flex: 1, padding: '0.5rem', border: '1px solid #ced4da', borderRadius: '4px', fontSize: '0.75rem', backgroundColor: 'white', color: '#495057', height: '36px' }} disabled={loading} placeholder="Enter GPS coordinates or use picker" />
                    <button type="button" onClick={() => setShowCoordinatePicker(true)} disabled={loading} style={{ padding: '0.5rem', backgroundColor: '#2d7c4a', color: 'white', border: 'none', borderRadius: '4px', cursor: loading ? 'not-allowed' : 'pointer', fontSize: '0.75rem', height: '36px', width: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Open coordinate picker">
                      <IoLocation />
                    </button>
                  </div>
      ...
      <CoordinatePickerModal isOpen={showCoordinatePicker} onClose={() => setShowCoordinatePicker(false)} onSelect={handleCoordinateSelect} initialCoordinates={formData.gps} />
    </>
  );
};
